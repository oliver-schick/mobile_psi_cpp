...
expanded_key lowmc_expand_key(lowmc_t const* lowmc, lowmc_key_t* key) {
  ...
  exp_key.key_0 = mzd_local_init_ex(1, lowmc->n, false);
  if (lowmc->m == 10) {
    exp_key.nl_part = mzd_local_init_ex(1, lowmc->r * 32, false);
  } else {
    exp_key.nl_part = mzd_local_init_ex(1, ((lowmc->r + 20) / 21) * 64, false);
  }

  MUL_MC(exp_key.nl_part, key, CONCAT(lowmc->precomputed_non_linear_part, matrix_postfix));
  XOR_MC(exp_key.nl_part, exp_key.nl_part, lowmc->precomputed_constant_non_linear);

  mzd_local_copy(exp_key.key_0, lowmc->precomputed_constant_linear);
  ADDMUL(exp_key.key_0, key, CONCAT(lowmc->k0, matrix_postfix));
#endif
  return exp_key;
}
#undef XOR
...
#endif
#if defined(WITH_LOWMC_128_128_208)
#include "lowmc_128_128_208.h"
#endif
...
#endif
#if defined(WITH_LOWMC_128_128_208)
#define LOWMC_INSTANCE_1 (&lowmc_128_128_208)
#endif
...
mzd_local_t* lowmc_call(lowmc_t const* lowmc, const expanded_key lowmc_key, mzd_local_t const* p) {
