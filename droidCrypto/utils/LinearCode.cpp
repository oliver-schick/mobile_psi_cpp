...
const uint16_t LinearCode::sLinearCodePlainTextMaxSize(64);
...
#ifndef NDEBUG
    if (line.size() != 2 * numCols - 1) throw std::runtime_error("");
#endif
    for (uint64_t j = 0; j < numCols; ++j) {
#ifndef NDEBUG
      if (line[j * 2] - '0' > 1) throw std::runtime_error("");
#endif
      buff[j] = line[j * 2] - '0';
      ;
    }
...
void LinearCode::load(const unsigned char *data, uint64_t size) {
...
void LinearCode::writeTextFile(std::ostream &out) {
  ...
  for (uint64_t row = 0; row < plaintextBitSize(); ++row) {
    BitIterator iter((uint8_t *)(mG.data() + row * codewordBlkSize()), 0);

    for (uint64_t col = 0; col < codewordBitSize() - 1; ++col) {
      out << *iter++ << " ";
    }

    if (iter.mByte > (uint8_t *)(mG.data() + mG.size()))
      throw std::runtime_error(LOCATION);
...
void LinearCode::writeBinCppFile(const std::string &fileName,
  ...
  int c = 0;
  out.read((char *)&c, 1);
  fOut << "static unsigned char " << name << "[] = { 0x" << std::hex
...
void LinearCode::generateMod8Table() {
  ...
  if (plaintextU8Size() > sLinearCodePlainTextMaxSize) {
    Log::e("LinCode", "here");
    //            std::cout << "The encode function assumes that the plaintext
    //            word"
    //                " size is less than " << sLinearCodePlainTextMaxSize << ".
    //                If" " this is not the case, raise the limit" << std::endl;
    throw std::runtime_error(LOCATION);
  }
  ...
  for (uint64_t i = 0; i < g8.bounds()[0]; ++i) {
    auto vv = g8[i];
          ...
          if (i * 8 + gRow < g.bounds()[0]) {
            for (uint64_t wordIdx = 0; wordIdx < codewordBlkSize(); ++wordIdx) {
              g8Block[g8Row][wordIdx] =
                  g8Block[g8Row][wordIdx] ^ g[i * 8 + gRow][wordIdx];
            }
          }
...
void LinearCode::encode(const span<block> &plaintxt,
  ...
  // span<uint8_t> pp((uint8_t*)plaintxt.data(), plaintextU8Size(), false);
  // span<uint8_t> cc((uint8_t*)codeword.data(), codewordU8Size(), false);
  // encode(pp, cc);
  encode((uint8_t *)plaintxt.data(), (uint8_t *)codeword.data());
  //
  //        std::array<block, 8>
  //            c{ ZeroBlock ,ZeroBlock ,ZeroBlock ,ZeroBlock,ZeroBlock
  //            ,ZeroBlock ,ZeroBlock ,ZeroBlock };
  //
  //
  ...
  //        // in a lookup table called mG8. Each sub-code takes up 256 *
  //        codeSize;
  //
  ...
  //                i = 0;
  //
  //            // this case has been optimized and we lookup two sub-codes at a
  //            time. uint64_t byteStep = 2; uint64_t kStep = rowSize *
  //            byteStep; for (; k < kStop; i += byteStep, k += kStep)
  //            {
  //                block* g0 = mG8.data() + k + byteView[i] * codeSize;
  //                block* g1 = mG8.data() + k + byteView[i + 1] * codeSize +
  //                rowSize;
  //#ifndef NDEBUG
  //                if (g1 >= mG8.data() + mG8.size())throw
  //                std::runtime_error("");
  //#endif
  //                c[0] = c[0] ^ g0[0];
  //                c[1] = c[1] ^ g0[1];
  ...
  //                {
  //
  //                    block* g0 = mG8.data() + j + byteView[i + 0] * codeSize
  //                    + rowSize * 0; block* g1 = mG8.data() + j + byteView[i +
  //                    1] * codeSize + rowSize * 1; block* g2 = mG8.data() + j
  //                    + byteView[i + 2] * codeSize + rowSize * 2; block* g3 =
  //                    mG8.data() + j + byteView[i + 3] * codeSize + rowSize *
  //                    3; block* g4 = mG8.data() + j + byteView[i + 4] *
  //                    codeSize + rowSize * 4; block* g5 = mG8.data() + j +
  //                    byteView[i + 5] * codeSize + rowSize * 5; block* g6 =
  //                    mG8.data() + j + byteView[i + 6] * codeSize + rowSize *
  //                    6; block* g7 = mG8.data() + j + byteView[i + 7] *
  //                    codeSize + rowSize * 7;
  //#ifndef NDEBUG
  //                    if (g7 >= mG8.data() + mG8.size())throw
  //                    std::runtime_error("");
  //#endif
  //
  //                    c[0] = c[0] ^ *g0;
...
void LinearCode::encode(uint8_t *input, uint8_t *codeword) {
  // highlevel idea: For each byte of the input, we have preprocessed
  // the corresponding partial code. That is, we have many sub-codes
  // each with input size 8. And these subcodes are precomputed
  // in a lookup table called mG8. Each sub-code takes up 256 * codeSize;

  const uint64_t &codeSize = mPow2CodeSize;
  const uint64_t rowSize = 256 * codeSize;
  const uint64_t rowSize8 = rowSize * 8;
  const uint64_t superRowCount = (mU8RowCount + 7) / 8;

  // in some cases below, the input array must be a
  // multiple of 8 in length...
  uint8_t _byteView[sLinearCodePlainTextMaxSize];
  uint8_t *byteView = _byteView;
  ...
  switch (codeSize) {
    case 1: {
      ...
      static const uint64_t byteStep = 8;
      block *g0 =
          mG8.data() +
          rowSize * 0;  // + k + byteView[i + 0] * codeSize + rowSize * 0;
      block *g1 =
          mG8.data() +
          rowSize * 1;  // + k + byteView[i + 1] * codeSize + rowSize * 1;
      block *g2 =
          mG8.data() +
          rowSize * 2;  // + k + byteView[i + 2] * codeSize + rowSize * 2;
      block *g3 =
          mG8.data() +
          rowSize * 3;  // + k + byteView[i + 3] * codeSize + rowSize * 3;
      block *g4 =
          mG8.data() +
          rowSize * 4;  // + k + byteView[i + 4] * codeSize + rowSize * 4;
      block *g5 =
          mG8.data() +
          rowSize * 5;  // + k + byteView[i + 5] * codeSize + rowSize * 5;
      block *g6 =
          mG8.data() +
          rowSize * 6;  // + k + byteView[i + 6] * codeSize + rowSize * 6;
      block *g7 =
          mG8.data() +
          rowSize * 7;  // + k + byteView[i + 7] * codeSize + rowSize * 7;

      uint64_t kStep = rowSize * byteStep;
      for (uint64_t i = 0; i < mPlaintextU8Size; i += byteStep) {
    ...
    case 2: {
      ...
      static const uint64_t byteStep = 4;

      uint64_t kStop = (mG8.size() / 8) * 8;
      uint64_t kStep = rowSize * byteStep;
      for (uint64_t k = 0, i = 0; k < kStop; i += byteStep, k += kStep) {
    ...
    case 4: {
      ...
      static const uint64_t byteStep = 2;

      int32_t kStop = (mG8.size() / 8) * 8;
      int32_t kStep = rowSize * byteStep;

      block *gg0 = mG8.data();
      block *gg1 = mG8.data() + rowSize;
      for (int32_t k = 0; k < kStop; byteView += byteStep, k += kStep) {
    ...
    case 8: {
      ...
      static const uint64_t byteStep = 1;
    ...
    default: {
      ...
      for (uint64_t j = 0; j < codeSize; ++j) {
        ...
        for (uint64_t i = 0; i < superRowCount; ++i, bv += 8) {
        ...
        memcpy((codeword + sizeof(block) * j), c,
               std::min<uint64_t>(sizeof(block),
                                  codewordU8Size() - j * sizeof(block)));
...
void LinearCode::encode_bch511(uint8_t *input, uint8_t *codeword) {
  ...
  const int32_t rowSize = 256 * codeSize;
